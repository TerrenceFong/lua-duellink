...
局内检测={}
局外检测={}
白龙 = {通召点 = false,贤士数 = 0,白石数 = 0,白石坐标={-1,-1},贤士坐标={-1,-1},少女坐标={-1,-1},摄理盖放=true,
其他通召=字符串转布尔(功能设置arr["多选-白龙-通召"]),
机龙开关=false,爆裂龙开关=false}
盖亚 = {开关 = false,通召点=false,技能开关  = false,魔道骑士数量=0,狱炎数量=0,士兵数量=0,征服数量 = 0,混沌数量=0,本源数量=0,怪数量=0,龙之镜数量=0,
魔道骑士开关=false,炼狱开关=false,士兵开关=false,本源开关=false,混沌开关 = false,魔导骑士坐标={-1,-1},诅咒之龙效果=false,怪效强制开关=false,士兵效果 = false}
...
function 局外检测.休闲入口()
	...
	return 识别.多点找色(23,300,23,1200,颜色_t,0,0.9)
end
...
function 局外检测.排名入口()
	...
	return 识别.多点找色(20,100,20,1200,颜色_t,0,0.9)
end
...
function 局内检测.怪兽识别(表示,范围序号,敌我) -- 标准: 表示: 攻守 范围序号 敌我
	...
	if 敌我 == "我" then
		if 识别.多点找色(范围_我怪[范围序号],颜色_我怪[范围序号],0,0.8) then
			if 表示 == "攻击" then
	...
	elseif 敌我 == "敌" then
		if 识别.多点找色(范围_敌怪[范围序号],颜色_敌怪[范围序号],0,0.8) then
			if 表示 == "攻击" then
...
function 局内操作.覆盖陷阱()
	...
	return 识别.识图(328,974,360,1005,"陷阱-放置",0.8,1)
end
